...
export const UltimasOrdenes = ({ entregas, control }) => {
  return (
                ...
                entregas.length === 0 ? (
                  ...
                  entregas?.flatMap((entrega, i) =>
                    entrega?.valuesEntregas?.flatMap((med, j) => (
                      med?.mvEntrega?.map((orden, k) => (
                        <tr key={`${i}-${j}-${k}`}>
                          <td className="border p-2 text-center">{med?.entrega?.prefijoEntrega}</td>
                          <td className="border p-2 text-center"> {moment(med?.entrega?.Fecha).locale("es").format("DD MMM YYYY")}</td>
                          <td className="border p-2 text-center">{orden?.Orden}</td>
                          <td className="border p-2 text-left">{orden?.Nombre}</td>
                          <td className="border p-2 text-center">{orden?.QtyOrden}</td>
                        </tr>
                      ))
                    ))
                  )
