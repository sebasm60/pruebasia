...
import { useMvPendientes } from "src/hooks/useMvPendientes";

import { InputField, InputSelect } from "src/components/molecules";
import { DataTable } from "../organisms/DataTable";
import { IconButton, ScreenTitle } from "../atoms";
...
export const PendientesTemplate = ({ title }) => {
  const [sedes, setSedes] = useState()
  const { data: pendientes, isLoading, isError, error, convenios } = useMvPendientes();
  ...
  const columns = [
    {
      header: 'Número',
      id: 'prefijoEntrega',
      accessorFn: row => `${row.Prefijo}${row.NoPendiente}`
    },
    {
      header: 'Fecha',
      accessorKey: 'Fh_Ing',
          ...
          <button
            onClick={() => {
              Swal({
                ...
                text: `¿Está seguro de cancelar el pendiente?`,
                icon: "info",
                dangerMode: true,
  ...
  if (isLoading) {
    return <div className="flex-1 flex justify-center items-center">Cargando pendientes...</div>;
  }
  ...
  return (
    <div className="flex flex-col h-full bg-white flex-1">
      ...
      <div className="flex-1 flex justify-center items-center p-3 flex-col">
        ...
        <DataTable data={pendientes.data} columns={columns} caption={'Tabla De Pendientes'} />
      </div>
    </div>
  );
}
