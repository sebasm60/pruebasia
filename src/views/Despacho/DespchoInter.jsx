...
import {
    ...
    getColorPendientes, getColorCambio
...
class DespachoInter extends Component {
    constructor(props) {
        super(props);
        this.state = {
            ...
            if (listdata != null) {
                ...
                if (listdata.pendientesOrders !== undefined) {
                    ...
                    listdata.pendientesOrders.forEach(item => {
                        //&& new Date(item.formula.fechaVencimiento).getTime() >= new Date().getTime()
                        if (item.formula.status === 'N') {
                ...
                if (listdata.orders.length > 0) {
                    ...
                    listdata.orders.forEach(item => {
                        ...
                        if (item.formula.status === 'N') {
                            ...
                            console.log(item.interop.aseguradora.plan);
                            console.log(this.state.listTipoPlanConsultado);

                            let existTP_Plan = this.state.listTipoPlanConsultado.find(i => i.value === item.interop.aseguradora.plan.codigo)
                            console.log(existTP_Plan);
                            if (existTP_Plan === undefined) {
                                tempArrayTP_Plan.push({
                                    value: item.interop.aseguradora.plan.codigo,
                                    label: item.interop.aseguradora.plan.nombre
                                    ...
                                    if (element.producto.medicamento?.Medicamento !== undefined) {
                                        ...
                                        getListSelect('Medicamento', '/' + element.producto.medicamento.IdMedicamento + '?mostrarSaldos=true&Bodega=' + localStorage.getItem('BodegaID') + '&IdConvenio=' + this.state.conveniosId.value, this.props.history)
                                            .then(listdataMed => {
                                                ...
                                                console.log(cuotaModeradora);
                                                console.log(valorMedicamentos);
                                                        ...
                                                        listdataMed.saldos.forEach(elementSaldo => {
                                                            if (elementSaldo.Lote !== null) {
                                                                ...
                                                                if (cantSaldoRecorrido === 0 && element.cantidadEntregada === 0) {
                                                                    ...
                                                                    console.log(item.interop.formula.cobro.valor);
                                                                    //if (cantDetalleOrden === 0) {
                                                                    this.setState({
                                                                        'sumValorTx': this.state.sumValorTx + (item.interop.formula.cobro.valor),
                                                                        sumNetoValorTx: this.state.sumNetoValorTx + (item.interop.formula.cobro.valor),
        ...
        validateRequiredFields(this.state, camposRequiredAcumulacionManual).then(result => {
            if (result) {
                ...
                if (this.state.MnMedicamento.Barras
                    ...
                    statusAlerta = 'alerta'
                    tempArrayAlert.push({
                        idMedicamento: this.state.MnMedicamento.MnPLU
                    });
                    this.setState({
                        'ListAlarmaMedicamentos': tempArray,
                        'cantAlarmas': tempArrayAlert
    ...
    onSaveEntrega() {
        ...
        if (this.state.cantAlarmas.length <= 0) {
            ...
            console.log(this.state.tipoId.value)
                        ...
                        this.state.ListNivel.map((item) => {
                            item.formula.tratamiento.map((itemTratamiento) => {
                                if (itemTratamiento.itemSelect === true || itemTratamiento.itemSelect === "true") {
                                    ...
                                    this.setState({'sumValorCMTx': this.state.sumValorCMTx + item.interop.formula.cobro.valor});
                                        ...
                                        validateRequiredFields(this.state, camposRequiredMedioPago).then(result => {
                                            if (result) {
                                                ...
                                                tempArrayHeader.push({
                                                    ...
                                                    IdConvenio: this.state.conveniosId.value,
                                                    IdPaciente: this.state.idPaciente,
                                                    IdCartera: this.state.tipoCarteraId.value,
                                                    FhNotificado: '',
                                                    Notificado: '',
                                                    Fh_Entrega: new Date(),
                                                    IdUsuario: localStorage.getItem('LoginUsers'),
    ...
    onPrintRecibo(resultado) {
        ...
        resultado.values.forEach(element => {
            this.setState({
                ...
                pdf_tiene_pendientes: element.mvPendiente.length > 0 ? 'block' : 'none',
                pdf_nroEntrega_pendientes: element.pendiente ? element.pendiente.NoPendiente : '',
                pdf_tpEntrega_pendientes: this.state.tipoEntregaId.label,
                pdf_TD_pendientes: element.mvPendiente.length ? '' : '',
                pdf_Pagado_pendientes: '0',
                pdf_PagadoCM_pendientes: '0',
                pdf_CARTERA_pendientes: element.pendiente ? element.pendiente.IdCartera : '',
                pdf_PLAN_pendientes: "POS",
                TpEntregaMvto_pendientes: element.mvPendiente ? element.mvPendiente : (element.mvPendientes ? element.mvPendiente : null),
                pdf_title_pendientes: element.mvPendiente.length ? 'PEDIDO' : '',
        ...
        return (
            <>
                <div className="flex flex-col h-full bg-white">
                    ...
                    <ScreenTitle title={`Dispensación interoperabilidad ${this.state.isDomicilio ? '-' + this.state.tpOrdenId : ''}`} />

                    <div className="flex-1 flex justify-center items-center">
                        <form className="bg-white p-2 rounded-lg shadow-lg w-full h-full flex flex-col gap-5">
                            <InputSelect
                                                                            ...
                                                                            'descripcion':
                                                                                (item) => (
                                                                                    <td style={{ maxWidth: '200px', minWidth: '200px' }}>
                                                                                        {item.producto?.medicamento !== undefined ? item.producto.medicamento.Medicamento.Nombre : 'NA'}
                                                                                    </td>
                                                                                ),
                                                                            'MarcaCcial':
                                                                                (item) => (
