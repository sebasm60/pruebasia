...
class BusquedaPlanes extends Component {
    constructor(props) {
        super(props);
        this.state = {
            IdMenu: 'Menu-planes',
    ...
    onSubmitFormFind() {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('Planes', '?page=1&limit=50&search=' + this.state.textBuscar.trim() + '', this.props.history).then(listdata => {
                    this.setState({
    ...
    onOpenActions(item, filter) {
        ...
        switch (filter) {
            default:
                ...
                break;
            case "Ingreso":
                this.props.history.push('/Maestros/Planes/_add');
                break;
            case "Consulta":
            case "Edicion":
                this.props.history.push('/Maestros/Planes/' + item.IdPlan + '');
                break;
            case "Retiro":
                DeleteItem(item.IdPlan, 'Planes', this.props.history);
                break;
            case "Inactivar":
            case "Activar":
                const datasend = {
                ...
                UpdateItem(item.IdPlan, datasend, 'Planes;put', this.props.history, '').then(result => {
                 this.onSubmitFormFind();
                });
