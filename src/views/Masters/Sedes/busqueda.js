...
class BusquedaSedes extends Component {
    constructor(props) {
        super(props);
        this.state = {
            IdMenu: 'Menu-sedes',
    ...
    onSubmitFormFind() {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('Sedes', '?page=1&limit=50&search=' + this.state.textBuscar.trim() + '', this.props.history).then(listdata => {
    ...
    onOpenActions(item, filter) {
        ...
        switch (filter) {
            default:
                ...
                break;
            case "Ingreso":
                this.props.history.push('/Maestros/Sedes/_add');
                break;
            case "Consulta":
            case "Edicion":
                this.props.history.push('/Maestros/Sedes/' + item.IdSede + '');
                break;
            case "Retiro":
                DeleteItem(item.IdSede, 'Sedes', this.props.history);
                break;
            case "Inactivar":
            case "Activar":
                const datasend = {
                ...
                UpdateItem(item.IdSede, datasend, 'Sedes;put', this.props.history, '').then(result => {
                 this.onSubmitFormFind();
                });
