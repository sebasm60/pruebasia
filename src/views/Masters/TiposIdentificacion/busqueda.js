...
class BusquedaTiposId extends Component {
    constructor(props) {
        super(props);
        this.state = {
            IdMenu: 'Menu-tiposid',
    ...
    onSubmitFormFind() {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('TiposId', '?page=1&limit=50&search=' + this.state.textBuscar.trim() + '', this.props.history).then(listdata => {
    ...
    onOpenActions(item, filter) {
        ...
        switch (filter) {
            default:
                ...
                break;
            case "Ingreso":
                this.props.history.push('/Maestros/TiposId/_add');
                break;
            case "Consulta":
            case "Edicion":
                this.props.history.push('/Maestros/TiposId/' + item.IdTipoId + '');
                break;
            case "Retiro":
                DeleteItem(item.IdTipoId, 'TiposId', this.props.history);
                break;
            case "Inactivar":
            case "Activar":
                const datasend = {
                ...
                UpdateItem(item.IdTipoId, datasend, 'TiposId;put', this.props.history, '').then(result => {
                 this.onSubmitFormFind();
                });
...
export default BusquedaTiposId