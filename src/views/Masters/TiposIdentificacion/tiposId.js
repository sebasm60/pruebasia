...
class tiposId extends Component {
    constructor(props) {
        super(props);
        this.state = {
            IdMenu: 'Menu-tiposid',
    ...
    onChangeInfo(id) {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('TiposId', '/' + id, this.props.history).then(listdata => {
                    console.log(listdata);
                    this.setState({
                        "showLoading": false,
                        "IdTipoId": listdata.IdTipoId,
            ...
            if (resultPermiss) {
                ...
                validateRequiredFields(this.state, camposRequired).then(result => {
                    if (result) {
                        ...
                        UpdateItem(this.state.IdTipoId, datasend, 'TiposId;put', history, '').then(result => {
                            this.onChangeInfo(this.state.IdTipoId);
                        });
                    }
                });
                this.setState({
    ...
    onSubmitFormDelete() {
        DeleteItem(this.state.IdTipoId, 'TiposId', this.props.history, '/Maestros/BusquedatiposId');
    }

    onGoNewItem() {
        localStorage.setItem('ActionItem', '');
        this.props.history.push('/Maestros/BusquedatiposId');
    }
    ...
    getTitle() {
        if (this.state.id === '_add') {
            return "Agregar TIPOS ID"
        } else {
            return "Consultar / Modificar / Eliminar TIPOS ID"
        }
    }
                                ...
                                <CCol md="12">
                                    <CForm inline>
                                        <CFormGroup className="pr-1" >
                                            <FloatingLabelInput id="IdTipoId" name="IdTipoId" style={{ paddingLeft: '2%' }} label="Id.TipoId" value={this.state.IdTipoId} onChange={this.onInputchange} onKeyDown={this.onInputchange} required />
                                        </CFormGroup>
                                        <CFormGroup className="pr-1" >
                                        ...
                                        <CFormGroup className="pr-1" style={{marginLeft: 'auto',marginRight: '0'}}  >
                                            ...
                                            <CButton
                                                ...
                                                variant="outline"
                                                shape="square"
                                                onClick={() => this.onDeleteItem(this.state.IdTipoId)}
