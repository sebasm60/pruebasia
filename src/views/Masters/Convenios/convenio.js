...
class convenio extends Component {
    ...
    onInputchange(event) {
        if (event.target.name.includes('ch')) {
            ...
            if (event.target.name === "IdConvenio") {
                this.setState({
    ...
    onChangeInfo(id) {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('Convenios', '/' + id, this.props.history).then(listdata => {
                    console.log(listdata);
                    this.setState({
                        "showLoading": false,
                        "IdConvenio": listdata.IdConvenio,
                        "Nombre": listdata.Nombre,
                        "chInterop": listdata.Interop === true ? true : false,
                        "IdListPrecio": listdata.IdListaPrecio,
                        "Nit": listdata.Nit,
            ...
            if (resultPermiss) {
                ...
                validateRequiredFields(this.state, camposRequired).then(result => {
                    if (result) {
                        ...
                        const datasend = {
                            IdConvenio: this.state.IdConvenio,
                            Nombre: this.state.Nombre,
                            Interop: this.state.chInterop,
                            Inactivo: false,
                            IdListaPrecio: this.state.IdListPrecio,
                            Nit: this.state.Nit,
                            FhCreacion: formatDateP(new Date(), 'yyyy-MM-DD'),
                            IdUser_Ing: localStorage.getItem('LoginUsers'),
                        }
                        AddItem(datasend, 'Convenios;post', history, '').then(result => {
                            this.ObtenerMestros();
                        });
                    }
                });
            ...
            if (resultPermiss) {
                ...
                validateRequiredFields(this.state, camposRequired).then(result => {
                    if (result) {
                        ...
                        UpdateItem(this.state.IdConvenio, datasend, 'Convenios;put', history, '').then(result => {
                            this.onChangeInfo(this.state.IdConvenio);
                        });
                    }
                });
                this.setState({
    ...
    onSubmitFormDelete() {
        DeleteItem(this.state.IdConvenio, 'Convenios', this.props.history, '/Maestros/BusquedaConv');
    }

    onGoNewItem() {
        localStorage.setItem('ActionItem', '');
        this.props.history.push('/Maestros/BusquedaConv');
    }
    ...
    getTitle() {
        if (this.state.id === '_add') {
            return "Agregar CONVENIOS"
        } else {
            return "Consultar / Modificar / Eliminar CONVENIOS"
        }
    }
                                ...
                                <CCol md="12">
                                    <CForm inline>
                                        <CFormGroup className="pr-1" >
                                            <FloatingLabelInput id="IdConvenio" name="IdConvenio" style={{ paddingLeft: '2%' }} label="Id.Convenio" value={this.state.IdConvenio} onChange={this.onInputchange} onKeyDown={this.onInputchange} required />
                                        </CFormGroup>
                                        <CFormGroup className="pr-1" >
                                        ...
                                        <CFormGroup className="pr-1" >
                                            <CInputCheckbox id="chInterop" name="chInterop" checked={this.state.chInterop}
                                                onChange={this.onInputchange} /> InterOperabilidad
                                        </CFormGroup>
                                        ...
                                        <CFormGroup className="pr-1" style={{ marginLeft: 'auto', marginRight: '0' }}  >
                                            ...
                                            <CButton
                                                ...
                                                variant="outline"
                                                shape="square"
                                                onClick={() => this.onDeleteItem(this.state.IdConvenio)}
                                    ...
                                    <CForm inline style={{ paddingTop: '0.8%' }}>
                                        <CFormGroup className="pr-1" >
                                            <FloatingLabelInput value={this.state.IdListPrecio} style={{ paddingLeft: '2%' }} onChange={this.onInputchange} label="Lista Precio" id="IdListPrecio" name="IdListPrecio" required="True" />
                                        </CFormGroup>
                                        <CFormGroup className="pr-1" >
                                            <FloatingLabelInput value={this.state.Nit} style={{ paddingLeft: '2%' }} onChange={this.onInputchange} label="Nit" id="Nit" name="Nit" required="True" />
                                        </CFormGroup>
                                    </CForm>
                                </CCol>
                            </CCardBody>
