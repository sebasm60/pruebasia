...
class consecutivos extends Component {
    ...
    onInputchange(event) {
        if (event.target.name === "IdConsecutivo") {
            this.setState({
    ...
    onChangeInfo(id) {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('Consecutivos', '/' + id, this.props.history).then(listdata => {
                    console.log(listdata);
                    this.setState({
                        "showLoading": false,
                        "IdConsecutivo": listdata.IdConsecutivo,
                        "Nombre": listdata.Nombre,
                        "Numero": listdata.Numero,
                        "Prefijo": listdata.Prefijo,
                        "TDMaestro": listdata.TDMaestro,
                        "chDispensario": listdata.Dispensario=== true ? true : false,
            ...
            if (resultPermiss) {
                ...
                validateRequiredFields(this.state, camposRequired).then(result => {
                    if (result) {
                        ...
                        const datasend = {
                            IdConsecutivo: this.state.IdConsecutivo,
                            Nombre: this.state.Nombre,
                            Numero: this.state.Numero,
                            Prefijo: this.state.Prefijo,
                            TDMaestro: this.state.TDMaestro,
                            Dispensario: this.state.Dispensario,
                            Inactivo: false,
                            FhCreacion: formatDateP(new Date(), 'yyyy-MM-DD'),
                            IdUser_Ing: localStorage.getItem('LoginUsers'),
                        }
                        AddItem(datasend, 'Consecutivos;post', history, '').then(result => {
                            this.ObtenerMestros();
                        });
                    }
                });
            ...
            if (resultPermiss) {
                ...
                validateRequiredFields(this.state, camposRequired).then(result => {
                    if (result) {
                        ...
                        UpdateItem(this.state.IdConsecutivo, datasend, 'Consecutivos;put', history, '').then(result => {
                            this.onChangeInfo(this.state.IdConsecutivo);
                        });
                    }
                });
                this.setState({
    ...
    onSubmitFormDelete() {
        DeleteItem(this.state.IdConsecutivo, 'Consecutivos', this.props.history, '/Maestros/BusquedaConsecutivos');
    }

    onGoNewItem() {
        localStorage.setItem('ActionItem', '');
        this.props.history.push('/Maestros/BusquedaConsecutivos');
    }
    ...
    getTitle() {
        if (this.state.id === '_add') {
            return "Agregar CONSECUTIVO"
        } else {
            return "Consultar / Modificar / Eliminar CONSECUTIVO"
        }
    }
                                ...
                                <CCol md="12">
                                    <CForm inline>
                                        <CFormGroup className="pr-1" >
                                            <FloatingLabelInput id="IdConsecutivo" name="IdConsecutivo" style={{ paddingLeft: '2%' }} label="Id.Consecutivo" value={this.state.IdConsecutivo} onChange={this.onInputchange} onKeyDown={this.onInputchange} required />
                                        </CFormGroup>
                                        <CFormGroup className="pr-1" >
                                        ...
                                        <CFormGroup className="pr-1" >
                                            <FloatingLabelInput value={this.state.Numero} style={{ paddingLeft: '2%' }} onChange={this.onInputchange} label="Numero" id="Numero" name="Numero" required="True" />
                                        </CFormGroup>
                                        
                                    </CForm>
                                    <CForm inline style={{ paddingTop: '0.8%' }}>
                                    <CFormGroup className="pr-1" >
                                            <FloatingLabelInput value={this.state.Prefijo} style={{ paddingLeft: '2%' }} onChange={this.onInputchange} label="Prefijo" id="Prefijo" name="Prefijo" required="True" />
                                        </CFormGroup>
                                        <CFormGroup className="pr-1" >
                                            <FloatingLabelInput value={this.state.TDMaestro} style={{ paddingLeft: '2%' }} onChange={this.onInputchange} label="TDMaestro" id="TDMaestro" name="TDMaestro" required="True" />
                                        </CFormGroup>
                                        <CFormGroup className="pr-1" >
                                            <CInputCheckbox id="chDispensario" name="chDispensario" checked={this.state.chDispensario}
                                                onChange={this.onInputchange} /> Dispensario
                                        </CFormGroup>
                                        <CFormGroup className="pr-1" style={{ marginLeft: 'auto', marginRight: '0' }}  >
                                            ...
                                            <CButton
                                                ...
                                                variant="outline"
                                                shape="square"
                                                onClick={() => this.onDeleteItem(this.state.IdConsecutivo)}
