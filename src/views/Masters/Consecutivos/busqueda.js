...
class BusquedaConsecutivos extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    onSubmitFormFind() {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('Consecutivos', '?page=1&limit=50&search=' + this.state.textBuscar.trim() + '', this.props.history).then(listdata => {
                    this.setState({
    ...
    onOpenActions(item, filter) {
        ...
        switch (filter) {
            default:
                ...
                break;
            case "Ingreso":
                this.props.history.push('/Maestros/Consecutivos/_add');
                break;
            case "Consulta":
            case "Edicion":
                this.props.history.push('/Maestros/Consecutivos/' + item.IdConsecutivo + '');
                break;
            case "Retiro":
                DeleteItem(item.IdConsecutivo, 'Consecutivos', this.props.history);
                break;
            case "Inactivar":
            case "Activar":
                const datasend = {
                ...
                UpdateItem(item.IdConsecutivo, datasend, 'Consecutivos;put', this.props.history, '').then(result => {
                 this.onSubmitFormFind();
                });
                        ...
                        <CCard style={{ display: this.state.showLoading ? 'none' : 'flex' }}>
                            ...
                            <CCardBody>
                                ...
                                <CDataTable
                                    ...
                                    fields={[
                                        { key: 'IdConsecutivo', label: 'IdConsecutivo' },
...
export default BusquedaConsecutivos