...
class BusquedaMedicamento extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    onSubmitFormFind() {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('Medicamento', '?page=1&limit=200&search=' + this.state.textBuscar.trim() + '', this.props.history).then(listdata => {
                    this.setState({
    ...
    onOpenActions(item, filter) {
        ...
        switch (filter) {
            default:
                ...
                break;
            case "Ingreso":
                this.props.history.push('/Maestros/medicamento/_add');
                break;
            case "Consulta":
            case "Edicion":
            case "Retiro":
                this.props.history.push('/Maestros/medicamento/' + item.IdMedicamento + '');
                break;
            case "Inactivar":
            case "Activar":
                const datasend = {
                ...
                UpdateItem(item.IdMedicamento, datasend, 'Medicamento;put', this.props.history, '').then(result => {
                    this.onSubmitFormFind();
                });
                break;
        }
...
export default BusquedaMedicamento