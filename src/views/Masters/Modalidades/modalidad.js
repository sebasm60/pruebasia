...
class ubicacion extends Component {
    ...
    onInputchange(event) {
        if (event.target.name === "IdModalidad") {
            this.setState({
    ...
    onChangeInfo(id) {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('modalidades', '/' + id, this.props.history).then(listdata => {
                    console.log(listdata);
                    this.setState({
                        "showLoading": false,
                        "IdModalidad": listdata.IdModalidad,
            ...
            if (resultPermiss) {
                ...
                validateRequiredFields(this.state, camposRequired).then(result => {
                    if (result) {
                        ...
                        UpdateItem(this.state.IdModalidad, datasend, 'modalidades;put', history, '').then(result => {
                            this.onChangeInfo(this.state.IdModalidad);
                        });
                    }
                });
                this.setState({
    ...
    onSubmitFormDelete() {
        DeleteItem(this.state.IdModalidad, 'modalidades', this.props.history, '/Maestros/BusquedaMod');
    }

    onGoNewItem() {
        localStorage.setItem('ActionItem', '');
        this.props.history.push('/Maestros/BusquedaMod');
    }
    ...
    getTitle() {
        if (this.state.id === '_add') {
            return "Agregar MODALIDADES"
        } else {
            return "Consultar / Modificar / Eliminar MODALIDADES"
        }
    }
                                ...
                                <CCol md="12">
                                    <CForm inline>
                                        <CFormGroup className="pr-1" >
                                            <FloatingLabelInput id="IdModalidad" name="IdModalidad" style={{ paddingLeft: '2%' }} label="Id.Modalidad" value={this.state.IdModalidad} onChange={this.onInputchange} onKeyDown={this.onInputchange} required />
                                        </CFormGroup>
                                        <CFormGroup className="pr-1" >
                                        ...
                                        <CFormGroup className="pr-1" style={{ marginLeft: 'auto', marginRight: '0' }}  >
                                            ...
                                            <CButton
                                                ...
                                                variant="outline"
                                                shape="square"
                                                onClick={() => this.onDeleteItem(this.state.IdModalidad)}
