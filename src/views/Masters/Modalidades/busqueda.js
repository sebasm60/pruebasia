...
class BusquedaModalidad extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    onSubmitFormFind() {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('Modalidades', '?page=1&limit=50&search=' + this.state.textBuscar.trim() + '', this.props.history).then(listdata => {
    ...
    onOpenActions(item, filter) {
        ...
        switch (filter) {
            default:
                ...
                break;
            case "Ingreso":
                this.props.history.push('/Maestros/Modalidades/_add');
                break;
            case "Consulta":
            case "Edicion":
                this.props.history.push('/Maestros/Modalidades/' + item.IdModalidad + '');
                break;
            case "Retiro":
                DeleteItem(item.IdModalidad, 'Modalidades', this.props.history);
                break;
            case "Inactivar":
            case "Activar":
                const datasend = {
                ...
                UpdateItem(item.IdModalidad, datasend, 'Modalidades;put', this.props.history, '').then(result => {
                    this.onSubmitFormFind();
                });
                        ...
                        <CCard style={{ display: this.state.showLoading ? 'none' : 'flex' }}>
                            ...
                            <CCardBody>
                                ...
                                <CDataTable
                                    ...
                                    fields={[
                                        { key: 'IdModalidad', label: 'IdModalidad' },
...
export default BusquedaModalidad