...
class BusquedaPaciente extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    componentDidMount() {
        ...
        let existFilter = localStorage.getItem(this.state.IdMenu + '_filter');
        console.log(existFilter);
        if (existFilter !== "" && existFilter !== null) {
            if (existFilter !== undefined) {
                this.setState({
                    "textBuscar": existFilter
                });
            }
        }
    ...
    onSubmitFormFind() {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                ...
                getListSelect('Paciente', '?page=1&searchFields=IdPaciente,Nombre&limit=200&search=' + this.state.textBuscar.trim() + '', this.props.history).then(listdata => {
                    this.setState({
                        "dataResult": listdata.data,
                        "showLoading": false
                    });
                });
                localStorage.setItem(this.state.IdMenu + '_filter', this.state.textBuscar.trim());
            } else {
    ...
    onOpenActions(item, filter) {
        ...
        localStorage.setItem('patient_creation_interpolarity', false);
        switch (filter) {
            default:
                swal("Advertencia", "Filtro de Accion no Encontrado", "warning");
                break;
            case "Ingreso":
                this.props.history.push('/Maestros/paciente/_add');
                break;
            case "Consulta":
            case "Edicion":
            case "Retiro":
                this.props.history.push('/Maestros/paciente/' + item.IdPaciente + '');
                break;
            case "Inactivar":
            case "Activar":
                const datasend = {
                ...
                UpdateItem(item.IdPaciente, datasend, 'Paciente;put', this.props.history, '').then(result => {
                    this.onSubmitFormFind();
                });
                break;
        }
