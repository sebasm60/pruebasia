...
class genero extends Component {
    ...
    onChangeInfo(id) {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('Generos', '/' + id, this.props.history).then(listdata => {
                    console.log(listdata);
                    this.setState({
                        "showLoading": false,
                        "IdGenero": listdata.IdGenero,
            ...
            if (resultPermiss) {
                ...
                validateRequiredFields(this.state, camposRequired).then(result => {
                    if (result) {
                        ...
                        AddItem(datasend, 'Generos;post', history, '/Maestros/Generos/' + this.state.IdGenero);
                    }
                });
                this.setState({
                    "showLoading": false
            ...
            if (resultPermiss) {
                ...
                validateRequiredFields(this.state, camposRequired).then(result => {
                    if (result) {
                        ...
                        UpdateItem(this.state.IdGenero, datasend, 'Generos;put', history, '').then(result => {
                            this.onChangeInfo(this.state.IdGenero);
                        });
                    }
                });
                this.setState({
    ...
    onSubmitFormDelete() {
        DeleteItem(this.state.IdGenero, 'Generos', this.props.history, '/Maestros/BusquedaGen');
    }

    onGoNewItem() {
        localStorage.setItem('ActionItem', '');
        this.props.history.push('/Maestros/BusquedaGen');
    }
    ...
    getTitle() {
        if (this.state.id === '_add') {
            return "Agregar GENERO"
        } else {
            return "Consultar / Modificar / Eliminar GENERO"
        }
    }
                                ...
                                <CCol md="12">
                                    <CForm inline>
                                        <CFormGroup className="pr-1" >
                                            <FloatingLabelInput id="IdGenero" name="IdGenero" style={{ paddingLeft: '2%' }} label="Id.Genero" value={this.state.IdGenero} onChange={this.onInputchange} onKeyDown={this.onInputchange} required />
                                        </CFormGroup>
                                        <CFormGroup className="pr-1" >
                                        ...
                                        <CFormGroup className="pr-1" style={{marginLeft: 'auto',marginRight: '0'}}  >
                                            ...
                                            <CButton
                                                ...
                                                variant="outline"
                                                shape="square"
                                                onClick={() => this.onDeleteItem(this.state.IdGenero)}
