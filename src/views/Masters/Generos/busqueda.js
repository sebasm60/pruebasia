...
class BusquedaGenero extends Component {
    constructor(props) {
        super(props);
        this.state = {
            IdMenu: 'Menu-generos',
    ...
    onSubmitFormFind() {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('Generos', '?page=1&limit=50&search=' + this.state.textBuscar.trim() + '', this.props.history).then(listdata => {
    ...
    onOpenActions(item, filter) {
        ...
        switch (filter) {
            default:
                ...
                break;
            case "Ingreso":
                this.props.history.push('/Maestros/Generos/_add');
                break;
            case "Consulta":
            case "Edicion":
                this.props.history.push('/Maestros/Generos/' + item.IdGenero + '');
                break;
            case "Retiro":
                DeleteItem(item.IdGenero, 'Generos', this.props.history);
                break;
            case "Inactivar":
            case "Activar":
                const datasend = {
                ...
                UpdateItem(item.IdGenero, datasend, 'Generos;put', this.props.history, '').then(result => {
                 this.onSubmitFormFind();
                });
