...
class regInvima extends Component {
    constructor(props) {
        super(props);
        this.state = {
            IdMenu: 'Menu-gruposrinv',
    ...
    onInputchange(event) {
        if (event.target.name === "IdGrupoRInv") {
            this.setState({
    ...
    onChangeInfo(id) {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('GRUPOSRINV', '/' + id, this.props.history).then(listdata => {
                    console.log(listdata);
                    this.setState({
                        "showLoading": false,
                        "IdGrupoRInv": listdata.IdGrupoRInv,
            ...
            if (resultPermiss) {
                ...
                validateRequiredFields(this.state, camposRequired).then(result => {
                    if (result) {
                        ...
                        UpdateItem(this.state.IdGrupoRInv, datasend, 'GRUPOSRINV;put', history, '').then(result => {
                            this.onChangeInfo(this.state.IdGrupoRInv);
                        });
                    }
                });
                this.setState({
    ...
    onSubmitFormDelete() {
        DeleteItem(this.state.IdGrupoRInv, 'GRUPOSRINV', this.props.history, '/Maestros/BusquedaGrupoRInv');
    }

    onGoNewItem() {
        localStorage.setItem('ActionItem', '');
        this.props.history.push('/Maestros/BusquedaGrupoRInv');
    }
    ...
    getTitle() {
        if (this.state.id === '_add') {
            return "Agregar GRUPOS REGISTRO INVIMA"
        } else {
            return "Consultar / Modificar / Eliminar GRUPOS REGISTRO INVIMA"
        }
    }
                                ...
                                <CCol md="12">
                                    <CForm inline>
                                        <CFormGroup className="pr-1" >
                                            <FloatingLabelInput id="IdGrupoRInv" name="IdGrupoRInv" style={{ paddingLeft: '2%' }} label="Id.GrupoRInv" value={this.state.IdGrupoRInv} onChange={this.onInputchange} onKeyDown={this.onInputchange} required />
                                        </CFormGroup>
                                        <CFormGroup className="pr-1" >
                                        ...
                                        <CFormGroup className="pr-1" style={{marginLeft: 'auto',marginRight: '0'}}  >
                                            ...
                                            <CButton
                                                ...
                                                variant="outline"
                                                shape="square"
                                                onClick={() => this.onDeleteItem(this.state.IdGrupoRInv)}
