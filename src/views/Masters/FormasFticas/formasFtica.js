...
class formaFtica extends Component {
    constructor(props) {
        super(props);
        this.state = {
            IdMenu: 'Menu-formasftica',
    ...
    onInputchange(event) {
        if (event.target.name === "IdFormaFtica") {
            this.setState({
    ...
    onChangeInfo(id) {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('FORMASFTICA', '/' + id, this.props.history).then(listdata => {
                    console.log(listdata);
                    this.setState({
                        "showLoading": false,
                        "IdFormaFtica": listdata.IdFormaFtica,
            ...
            if (resultPermiss) {
                ...
                validateRequiredFields(this.state, camposRequired).then(result => {
                    if (result) {
                        ...
                        const datasend = {
                            IdFormaFtica: this.state.IdFormaFtica,
                            Nombre: this.state.Nombre,
                            Inactivo: false,
                            FhCreacion: formatDateP(new Date(), 'yyyy-MM-DD'),
                            IdUser_Ing: localStorage.getItem('LoginUsers'),
                        }
                        AddItem(datasend, 'FORMASFTICA;post', history, '').then(result => {
                            this.ObtenerMestros();
                        });
                    }
                });
            ...
            if (resultPermiss) {
                ...
                validateRequiredFields(this.state, camposRequired).then(result => {
                    if (result) {
                        ...
                        UpdateItem(this.state.IdFormaFtica, datasend, 'FORMASFTICA;put', history, '').then(result => {
                            this.onChangeInfo(this.state.IdFormaFtica);
                        });
                    }
                });
                this.setState({
    ...
    onSubmitFormDelete() {
        DeleteItem(this.state.IdFormaFtica, 'FORMASFTICA', this.props.history, '/Maestros/BusquedaFormFtica');
    }
    onGoNewItem() {
        localStorage.setItem('ActionItem', '');
        this.props.history.push('/Maestros/BusquedaFormFtica');
    }
    ...
    getTitle() {
        if (this.state.id === '_add') {
            return "Agregar FORMAS FARMACEUTICAS"
        } else {
            return "Consultar / Modificar / Eliminar FORMAS FARMACEUTICAS"
        }
    }
                                ...
                                <CCol md="12">
                                    <CForm inline>
                                        <CFormGroup className="pr-1" >
                                            <FloatingLabelInput id="IdFormaFtica" name="IdFormaFtica" style={{ paddingLeft: '2%' }} label="Id.FormaFtica" value={this.state.IdFormaFtica} onChange={this.onInputchange} onKeyDown={this.onInputchange} required />
                                        </CFormGroup>
                                        <CFormGroup className="pr-1" >
                                        ...
                                        <CFormGroup className="pr-1" style={{marginLeft: 'auto',marginRight: '0'}}  >
                                            ...
                                            <CButton
                                                ...
                                                variant="outline"
                                                shape="square"
                                                onClick={() => this.onDeleteItem(this.state.IdFormaFtica)}
