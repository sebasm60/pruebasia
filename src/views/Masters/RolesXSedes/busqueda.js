...
class BusquedaRolesSedes extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    onSubmitFormFind() {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('RolesSedes', '?search=' + this.state.textBuscar.trim() + '', this.props.history).then(listdata => {
    ...
    onOpenActions(item, filter) {
        ...
        switch (filter) {
            default:
                ...
                break;
            case "Ingreso":
                this.props.history.push('/Maestros/RolesSedes/_add/_add');
                break;
            case "Consulta":
            case "Edicion":
                this.props.history.push('/Maestros/RolesSedes/' + item.IdSede +'/'+item.IdRol+ '');
                break;
            case "Retiro":
                DeleteItem('Sede/'+item.IdSede+'/rol/'+item.IdRol, 'RolesSedes', this.props.history);
                break;
            case "Inactivar":
            case "Activar":
                const datasend = {
                ...
                UpdateItem(item.IdRol, datasend, 'RolesSedes;put', this.props.history, '').then(result => {
                 this.onSubmitFormFind();
                });
                        ...
                        <CCard style={{ display: this.state.showLoading ? 'none' : 'flex' }}>
                            ...
                            <CCardBody>
                                ...
                                <CDataTable
                                    ...
                                    fields={[
                                        { key: 'IdSede', label: 'IdSede' },
                                        { key: 'NombreSede', label: 'Nombre Sede' },
                                        { key: 'IdRol', label: 'IdRol' },
                                        { key: 'NombreRol', label: 'Nombre Rol' },
                                        { key: 'IdBodega', label: 'IdBodega' },
                                        {
                                            key: 'Acciones',
                                            label: '',
                                            _style: { width: '1%' },
                                    ...
                                    scopedSlots={{
                                        'NombreSede':
                                            (item) => (
                                                <td>
                                                    {toUPPER(item.Sede.Nombre)}
                                                </td>
                                            ),
                                        'NombreRol':
                                            (item) => (
                                                <td>
                                                    {toUPPER(item.Rol.Nombre)}
                                                </td>
                                            ),
                                        'Acciones':
                                            (item, index) => {
...
export default BusquedaRolesSedes