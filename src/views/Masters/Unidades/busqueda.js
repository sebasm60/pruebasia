...
class BusquedaUnidad extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    onSubmitFormFind() {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('Unidades', '?page=1&limit=50&search=' + this.state.textBuscar.trim() + '', this.props.history).then(listdata => {
    ...
    onOpenActions(item, filter) {
        ...
        switch (filter) {
            default:
                ...
                break;
            case "Ingreso":
                this.props.history.push('/Maestros/Unidades/_add');
                break;
            case "Consulta":
            case "Edicion":
                this.props.history.push('/Maestros/Unidades/' + item.IdUnidad + '');
                break;
            case "Retiro":
                DeleteItem(item.IdUnidad, 'Unidades', this.props.history);
                break;
            case "Inactivar":
            case "Activar":
                const datasend = {
                ...
                UpdateItem(item.IdUnidad, datasend, 'Unidades;put', this.props.history, '').then(result => {
                 this.onSubmitFormFind();
                });
