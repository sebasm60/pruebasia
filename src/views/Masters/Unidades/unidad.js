...
class ubicacion extends Component {
    ...
    onChangeInfo(id) {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('unidades', '/' + id, this.props.history).then(listdata => {
                    console.log(listdata);
                    this.setState({
                        "showLoading": false,
                        "IdUnidad": listdata.IdUnidad,
            ...
            if (resultPermiss) {
                ...
                validateRequiredFields(this.state, camposRequired).then(result => {
                    if (result) {
                        ...
                        AddItem(datasend, 'unidades;post', history, '').then(result => {
                            this.ObtenerMestros();
                        });
                    }
                });
            ...
            if (resultPermiss) {
                ...
                validateRequiredFields(this.state, camposRequired).then(result => {
                    if (result) {
                        ...
                        UpdateItem(this.state.IdUnidad, datasend, 'unidades;put', history, '').then(result => {
                            this.onChangeInfo(this.state.IdUnidad);
                        });
                    }
                });
                this.setState({
    ...
    onSubmitFormDelete() {
        DeleteItem(this.state.IdUnidad, 'unidades', this.props.history, '/Maestros/BusquedaUni');
    }

    onGoNewItem() {
        localStorage.setItem('ActionItem', '');
        this.props.history.push('/Maestros/BusquedaUni');
    }
                                ...
                                <CCol md="12">
                                    <CForm inline>
                                        <CFormGroup className="pr-1" >
                                            <FloatingLabelInput id="IdUnidad" name="IdUnidad" style={{ paddingLeft: '2%' }} label="Id.Unidad" value={this.state.IdUnidad} onChange={this.onInputchange} onKeyDown={this.onInputchange} required />
                                        </CFormGroup>
                                        <CFormGroup className="pr-1" >
                                        ...
                                        <CFormGroup className="pr-1" style={{marginLeft: 'auto',marginRight: '0'}}  >
                                            ...
                                            <CButton
                                                ...
                                                variant="outline"
                                                shape="square"
                                                onClick={() => this.onDeleteItem(this.state.IdUnidad)}
