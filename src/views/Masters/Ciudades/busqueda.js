...
class BusquedaCiudades extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    onSubmitFormFind() {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('Ciudades', '?page=1&limit=50&search=' + this.state.textBuscar.trim() + '', this.props.history).then(listdata => {
    ...
    onOpenActions(item, filter) {
        ...
        switch (filter) {
            default:
                ...
                break;
            case "Ingreso":
                this.props.history.push('/Maestros/Ciudades/_add');
                break;
            case "Consulta":
            case "Edicion":
                this.props.history.push('/Maestros/Ciudades/' + item.IdCiudad + '');
                break;
            case "Retiro":
                DeleteItem(item.IdCiudad, 'Ciudades', this.props.history);
                break;
            case "Inactivar":
            case "Activar":
                const datasend = {
                ...
                UpdateItem(item.IdCiudad, datasend, 'Ciudades;put', this.props.history, '').then(result => {
                 this.onSubmitFormFind();
                });
...
export default BusquedaCiudades