...
class ciudades extends Component {
    ...
    ObtenerMestros() {
        ...
        getListSelect('Departamentos', '?Inactivo=false', history).then(listdata => {
            let tempArray = [];
            ...
            this.setState({
                "listDepartamentos": tempArray
            });
            if (this.state.id !== '_add') {
                this.setState({
                    "IdCiudad": this.state.id,
                    "showLoading": true
                });
    ...
    onChangeInfo(id) {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('Ciudades', '/' + id, this.props.history).then(listdata => {
                    console.log(listdata);
                    this.setState({
                        "showLoading": false,
                        "IdCiudad": listdata.IdCiudad,
                        "Nombre": listdata.Nombre,
                        "departamentoId": this.state.listDepartamentos.find(i => i.value === listdata.IdDepartamento),
            ...
            if (resultPermiss) {
                ...
                validateRequiredFields(this.state, camposRequired).then(result => {
                    if (result) {
                        ...
                        AddItem(datasend, 'Ciudades;post', history, '').then(result => {
                            this.ObtenerMestros();
                        });
                    }
                });
            ...
            if (resultPermiss) {
                ...
                validateRequiredFields(this.state, camposRequired).then(result => {
                    if (result) {
                        ...
                        UpdateItem(this.state.IdCiudad, datasend, 'Ciudades;put', history, '').then(result => {
                            this.onChangeInfo(this.state.IdCiudad);
                        });
                    }
                });
                this.setState({
    ...
    onSubmitFormDelete() {
        DeleteItem(this.state.IdCiudad, 'Ciudades', this.props.history, '/Maestros/BusquedaCiudades');
    }

    onGoNewItem() {
        localStorage.setItem('ActionItem', '');
        this.props.history.push('/Maestros/BusquedaCiudades');
    }
    ...
    getTitle() {
        if (this.state.id === '_add') {
            return "Agregar CIUDADES"
        } else {
            return "Consultar / Modificar / Eliminar CIUDADES"
        }
    }
                                ...
                                <CCol md="12">
                                    <CForm inline>
                                        <CFormGroup className="pr-1" >
                                            <FloatingLabelInput id="IdCiudad" name="IdCiudad" style={{ paddingLeft: '2%' }} label="Id.Ciudad" value={this.state.IdCiudad} onChange={this.onInputchange} onKeyDown={this.onInputchange} required />
                                        </CFormGroup>
                                        <CFormGroup className="pr-1" >
                                        ...
                                        <CFormGroup className="pr-1" >
                                            <div style={{ width: '324px' }} >
                                                <Select
                                                    options={this.state.listDepartamentos}
                                                    ...
                                                    onChange={event => this.onSelectchange("departamentoId", event)}
                                                ></Select>
                                            </div>
                                        </CFormGroup>
                                        <CFormGroup className="pr-1" style={{marginLeft: 'auto',marginRight: '0'}}  >
                                            ...
                                            <CButton
                                                ...
                                                variant="outline"
                                                shape="square"
                                                onClick={() => this.onDeleteItem(this.state.IdCiudad)}
