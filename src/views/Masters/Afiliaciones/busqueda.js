...
class BusquedaAfiliacion extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    onSubmitFormFind() {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('Afiliaciones', '?page=1&limit=50&search=' + this.state.textBuscar.trim() + '', this.props.history).then(listdata => {
    ...
    onOpenActions(item, filter) {
        ...
        switch (filter) {
            ...
            case "Edicion":
            case "Retiro":
                this.props.history.push('/Maestros/afiliacion/' + item.IdAfiliacion + '');
                break;
            case "Inactivar":
            case "Activar":
                const datasend = {
                ...
                UpdateItem(item.IdAfiliacion, datasend, 'Afiliaciones;put', this.props.history, '').then(result => {
                    this.onSubmitFormFind();
                });
                break;
        }
                        ...
                        <CCard style={{ display: this.state.showLoading ? 'none' : 'flex' }}>
                            ...
                            <CCardBody>
                                ...
                                <CDataTable
                                    ...
                                    fields={[
                                        { key: 'IdAfiliacion', label: 'IdAfiliacion' },
...
export default BusquedaAfiliacion