...
class afiliacion extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    onInputchange(event) {
        if (event.target.name === "IdAfiliacion") {
            this.setState({
    ...
    onChangeInfo(id) {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('Afiliaciones', '/' + id, this.props.history).then(listdata => {
                    console.log(listdata);
                    this.setState({
                        "showLoading": false,
                        "IdAfiliacion": listdata.IdAfiliacion,
            ...
            if (resultPermiss) {
                ...
                validateRequiredFields(this.state, camposRequired).then(result => {
                    if (result) {
                        ...
                        const datasend = {
                            IdAfiliacion: this.state.IdAfiliacion,
                            Nombre: this.state.Nombre,
                            Inactivo: false,
                            FhCreacion: formatDateP(new Date(), 'yyyy-MM-DD'),
                            IdUser_Ing: localStorage.getItem('LoginUsers'),
                        }
                        AddItem(datasend, 'Afiliaciones;post', history, '/Maestros/BusquedaAfil').then(result => {
                            this.ObtenerMestros();
                        });
                    }
                });
            ...
            if (resultPermiss) {
                ...
                validateRequiredFields(this.state, camposRequired).then(result => {
                    if (result) {
                        ...
                        UpdateItem(this.state.IdAfiliacion, datasend, 'Afiliaciones;put', history, '').then(result => {
                            this.onChangeInfo(this.state.IdAfiliacion);
                        });
                    }
                });
                this.setState({
    ...
    onSubmitFormDelete() {
        DeleteItem(this.state.IdAfiliacion, 'Afiliaciones', this.props.history,'/Maestros/BusquedaAfil');
    }

    onGoNewItem() {
        localStorage.setItem('ActionItem', '');
        this.props.history.push('/Maestros/BusquedaAfil');
    }
    ...
    getTitle() {
        if (this.state.id === '_add') {
            return "Agregar AFILIACIONES"
        } else {
            return "Consultar / Modificar / Eliminar AFILIACIONES"
        }
    }
                                ...
                                <CCol md="12">
                                    <CForm inline>
                                        <CFormGroup className="pr-1" >
                                            <FloatingLabelInput id="IdAfiliacion" name="IdAfiliacion" style={{ paddingLeft: '2%' }} label="Id.Afiliacion" value={this.state.IdAfiliacion} onChange={this.onInputchange} onKeyDown={this.onInputchange} required />
                                        </CFormGroup>
                                        <CFormGroup className="pr-1" >
                                        ...
                                        <CFormGroup className="pr-1" style={{ marginLeft: 'auto', marginRight: '0' }}  >
                                            ...
                                            <CButton
                                                ...
                                                variant="outline"
                                                shape="square"
                                                onClick={() => this.onDeleteItem(this.state.IdAfiliacion)}
