...
class regInvimaEstado extends Component {
    constructor(props) {
        super(props);
        this.state = {
            IdMenu: 'Menu-estadosrinv',
    ...
    onInputchange(event) {
        if (event.target.name === "IdEstadoRInv") {
            this.setState({
    ...
    onChangeInfo(id) {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('ESTADOSRINV', '/' + id, this.props.history).then(listdata => {
                    console.log(listdata);
                    this.setState({
                        "showLoading": false,
                        "IdEstadoRInv": listdata.IdEstadoRInv,
            ...
            if (resultPermiss) {
                ...
                validateRequiredFields(this.state, camposRequired).then(result => {
                    if (result) {
                        ...
                        const datasend = {
                            IdEstadoRInv: this.state.IdEstadoRInv,
                            Nombre: this.state.Nombre,
                            Inactivo: false,
                            FhCreacion: formatDateP(new Date(), 'yyyy-MM-DD'),
                            IdUser_Ing: localStorage.getItem('LoginUsers'),
                        }
                        AddItem(datasend, 'ESTADOSRINV;post', history, '').then(result => {
                            this.ObtenerMestros();
                        });
                    }
                });
            ...
            if (resultPermiss) {
                ...
                validateRequiredFields(this.state, camposRequired).then(result => {
                    if (result) {
                        ...
                        UpdateItem(this.state.IdEstadoRInv, datasend, 'ESTADOSRINV;put', history, '').then(result => {
                            this.onChangeInfo(this.state.IdEstadoRInv);
                        });
                    }
                });
                this.setState({
    ...
    onSubmitFormDelete() {
        DeleteItem(this.state.IdEstadoRInv, 'ESTADOSRINV', this.props.history, '/Maestros/BusquedaEstadoRInv');
    }

    onGoNewItem() {
        localStorage.setItem('ActionItem', '');
        this.props.history.push('/Maestros/BusquedaEstadoRInv');
    }
    ...
    getTitle() {
        if (this.state.id === '_add') {
            return "Agregar ESTADOS REGISTRO INVIMA"
        } else {
            return "Consultar / Modificar / Eliminar ESTADOS REGISTRO INVIMA"
        }
    }
                                ...
                                <CCol md="12">
                                    <CForm inline>
                                        <CFormGroup className="pr-1" >
                                            <FloatingLabelInput id="IdEstadoRInv" name="IdEstadoRInv" style={{ paddingLeft: '2%' }} label="Id.EstadoRInv" value={this.state.IdEstadoRInv} onChange={this.onInputchange} onKeyDown={this.onInputchange} required />
                                        </CFormGroup>
                                        <CFormGroup className="pr-1" >
                                        ...
                                        <CFormGroup className="pr-1" style={{marginLeft: 'auto',marginRight: '0'}}  >
                                            ...
                                            <CButton
                                                ...
                                                variant="outline"
                                                shape="square"
                                                onClick={() => this.onDeleteItem(this.state.IdEstadoRInv)}
...
export default regInvimaEstado