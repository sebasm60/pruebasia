...
class BusquedaPaises extends Component {
    constructor(props) {
        super(props);
        this.state = {
            IdMenu: 'Menu-paises',
    ...
    onSubmitFormFind() {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                ...
                getListSelect('Paises', '?page=1&limit=50&search=' + this.state.textBuscar.trim() + '', this.props.history).then(listdata => {
    ...
    onOpenActions(item, filter) {
        ...
        switch (filter) {
            default:
                ...
                break;
            case "Ingreso":
                this.props.history.push('/Maestros/Pais/_add');
                break;
            case "Consulta":
            case "Edicion":
            case "Retiro":
                this.props.history.push('/Maestros/Pais/' + item.IdPais + '');
                break;
            case "Inactivar":
            case "Activar":
                const datasend = {
                ...
                UpdateItem(item.IdPais, datasend, 'Paises;put', this.props.history, '').then(result => {
                    this.onSubmitFormFind();
                });
                break;
        }
