...
class BusquedaMenu extends Component {
    constructor(props) {
        super(props);
        this.state = {
            IdMenu: 'Menu-menus',
    ...
    onSubmitFormFind() {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('Menus', '?page=1&limit=50&search=' + this.state.textBuscar.trim() + '', this.props.history).then(listdata => {
    ...
    onOpenActions(item, filter) {
        ...
        switch (filter) {
            default:
                ...
                break;
            case "Ingreso":
                this.props.history.push('/Maestros/menu/_add');
                break;
            case "Consulta":
            case "Edicion":
                this.props.history.push('/Maestros/menu/' + item.IdMenu + '');
                break;
            case "Retiro":
                DeleteItem(item.IdMenu, 'Menus', this.props.history);
                break;
            case "Inactivar":
            case "Activar":
                const datasend = {
                ...
                UpdateItem(item.IdMenu, datasend, 'Menus;put', this.props.history, '').then(result => {
                 this.onSubmitFormFind();
                });
