...
class menu extends Component {
    ...
    componentDidMount() {
        getPermissions(this.state.IdMenuP, 'Ingreso', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                this.ObtenerMestros();
            } else {
    ...
    onChangeInfo(id) {
        ...
        getPermissions(this.state.IdMenuP, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('Menus', '/' + id, this.props.history).then(listdata => {
                    console.log(listdata);
                    this.setState({
                        "showLoading": false,
                        "IdMenu": listdata.IdMenu,
                        "Nombre": listdata.Nombre,
                        "Posicion": listdata.Posicion,
                        "chIERC": listdata.IERC=== true ? true : false,
                ...
                validateRequiredFields(this.state, camposRequired).then(result => {
                    if (result) {
                        ...
                        const datasend = {
                            ...
                            IERC: this.state.chIERC=== true ? true : false,
                            Inactivo: false,
                            FhCreacion: formatDateP(new Date(), 'yyyy-MM-DD'),
                            IdUser_Ing: localStorage.getItem('LoginUsers'),
                        }
                        AddItem(datasend, 'Menus;post', history, '/Maestros/Menus/' + this.state.IdMenu);
                    }
                });
                this.setState({
                    "showLoading": false
    ...
    onSubmitFormUpdate() {
        getPermissions(this.state.IdMenuP, 'Edicion', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                this.setState({
                    "showLoading": true
                });
                validateRequiredFields(this.state, camposRequired).then(result => {
                    if (result) {
                        ...
                        UpdateItem(this.state.IdMenu, datasend, 'Menus;put', history, '').then(result => {
                            this.onChangeInfo(this.state.IdMenu);
                        });
                    }
                });
                this.setState({
    ...
    onSubmitFormDelete() {
        DeleteItem(this.state.IdMenu, 'Menus', this.props.history, '/Maestros/BusquedaGen');
    }

    onGoNewItem() {
        localStorage.setItem('ActionItem', '');
        this.props.history.push('/Maestros/BusquedaMenus');
    }

    onDeleteItem(id) {
        getPermissions(this.state.IdMenuP, 'Retiro', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                const { history } = this.props;
    ...
    getTitle() {
        if (this.state.id === '_add') {
            ...
            return "Consultar / Modificar / Eliminar MENU"
        }
    }
                                ...
                                <CCol md="12">
                                    <CForm inline>
                                        <CFormGroup className="pr-1" >
                                            <FloatingLabelInput id="IdMenu" name="IdMenu" style={{ paddingLeft: '2%' }} label="Id.Menu" value={this.state.IdMenu} onChange={this.onInputchange} onKeyDown={this.onInputchange} required />
                                        </CFormGroup>
                                        <CFormGroup className="pr-1" >
                                    ...
                                    <CForm inline style={{ paddingTop: '0.8%' }}>
                                        <CFormGroup className="pr-1" >
                                            <FloatingLabelInput value={this.state.Posicion} style={{ paddingLeft: '2%' }} type='number' onChange={this.onInputchange} label="Posicion" id="Posicion" name="Posicion" required="True" />
                                        </CFormGroup>
                                        <CFormGroup className="pr-1" >
                                            <CInputCheckbox id="chIERC" name="chIERC" checked={this.state.chIERC}
                                                onChange={this.onInputchange} /> IERC
                                        </CFormGroup>
                                       {/*  <CFormGroup className="pr-1" style={{ marginLeft: 'auto', marginRight: '0' }}  >
                                            ...
                                            <CButton
                                                ...
                                                variant="outline"
                                                shape="square"
                                                onClick={() => this.onDeleteItem(this.state.IdMenu)}
