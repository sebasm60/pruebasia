...
class regimen extends Component {
    ...
    onInputchange(event) {
        if (event.target.name === "IdRegimen") {
            this.setState({
    ...
    onChangeInfo(id) {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('regimenes', '/' + id, this.props.history).then(listdata => {
                    console.log(listdata);
                    this.setState({
                        "showLoading": false,
                        "IdRegimen": listdata.IdRegimen,
            ...
            if (resultPermiss) {
                ...
                validateRequiredFields(this.state, camposRequired).then(result => {
                    if (result) {
                        ...
                        UpdateItem(this.state.IdRegimen, datasend, 'regimenes;put', history, '').then(result => {
                            this.onChangeInfo(this.state.IdRegimen);
                        });
                    }
                });
                this.setState({
    ...
    onSubmitFormDelete() {
        DeleteItem(this.state.IdRegimen, 'regimenes', this.props.history, '/Maestros/BusquedaReg');
    }

    onGoNewItem() {
        localStorage.setItem('ActionItem', '');
        this.props.history.push('/Maestros/BusquedaReg');
    }
    ...
    getTitle() {
        if (this.state.id === '_add') {
            return "Agregar REGIMENES"
        } else {
            return "Consultar / Modificar / Eliminar REGIMENES"
        }
    }
                                ...
                                <CCol md="12">
                                    <CForm inline>
                                        <CFormGroup className="pr-1" >
                                            <FloatingLabelInput id="IdRegimen" name="IdRegimen" style={{ paddingLeft: '2%' }} label="Id.Regimen" value={this.state.IdRegimen} onChange={this.onInputchange} onKeyDown={this.onInputchange} required />
                                        </CFormGroup>
                                        <CFormGroup className="pr-1" >
                                        ...
                                        <CFormGroup className="pr-1" style={{marginLeft: 'auto',marginRight: '0'}}  >
                                            ...
                                            <CButton
                                                ...
                                                variant="outline"
                                                shape="square"
                                                onClick={() => this.onDeleteItem(this.state.IdRegimen)}
