...
class BusquedaUsuario extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    onSubmitFormFind() {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('Usuarios', '?page=1&limit=50&search=' + this.state.textBuscar.trim() + '', this.props.history).then(listdata => {
    ...
    onOpenActions(item, filter) {
        ...
        switch (filter) {
            default:
                ...
                break;
            case "Ingreso":
                this.props.history.push('/Maestros/Usuarios/_add');
                break;
            case "Consulta":
            case "Edicion":
            case "Retiro":
                this.props.history.push('/Maestros/Usuarios/' + item.Id + '');
                break;
            case "Inactivar":
            case "Activar":
                const datasend = {
                ...
                UpdateItem(item.IdUsuario, datasend, 'Usuarios;put', this.props.history, '').then(result => {
                    this.onSubmitFormFind();
                });
