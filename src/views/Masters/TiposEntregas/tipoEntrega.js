...
class tiposEntrega extends Component {
    ...
    onInputchange(event) {
        if (event.target.name === "IdTipoEntrega") {
            this.setState({
    ...
    onChangeInfo(id) {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('TiposEntrega', '/' + id, this.props.history).then(listdata => {
                    console.log(listdata);
                    this.setState({
                        "showLoading": false,
                        "IdTipoEntrega": listdata.IdTipoEntrega,
            ...
            if (resultPermiss) {
                ...
                validateRequiredFields(this.state, camposRequired).then(result => {
                    if (result) {
                        ...
                        const datasend = {
                            IdTipoEntrega: this.state.IdTipoEntrega,
                            Nombre: this.state.Nombre,
                            Inactivo: false,
                            FhCreacion: formatDateP(new Date(), 'yyyy-MM-DD'),
                            IdUser_Ing: localStorage.getItem('LoginUsers'),
                        }
                        AddItem(datasend, 'TiposEntrega;post', history, '').then(result => {
                            this.ObtenerMestros();
                        });
                    }
                });
            ...
            if (resultPermiss) {
                ...
                validateRequiredFields(this.state, camposRequired).then(result => {
                    if (result) {
                        ...
                        UpdateItem(this.state.IdTipoEntrega, datasend, 'TiposEntrega;put', history, '').then(result => {
                            this.onChangeInfo(this.state.IdTipoEntrega);
                        });
                    }
                });
                this.setState({
    ...
    onSubmitFormDelete() {
        DeleteItem(this.state.IdTipoEntrega, 'TiposEntrega', this.props.history, '/Maestros/BusquedaTiposEnt');
    }

    onGoNewItem() {
        localStorage.setItem('ActionItem', '');
        this.props.history.push('/Maestros/BusquedaTiposEnt');
    }
    ...
    getTitle() {
        if (this.state.id === '_add') {
            return "Agregar TIPO ENTREGA"
        } else {
            return "Consultar / Modificar / Eliminar TIPO ENTREGA"
        }
    }
                                ...
                                <CCol md="12">
                                    <CForm inline>
                                        <CFormGroup className="pr-1" >
                                            <FloatingLabelInput id="IdTipoEntrega" name="IdTipoEntrega" style={{ paddingLeft: '2%' }} label="Id.TipoEntrega" value={this.state.IdTipoEntrega} onChange={this.onInputchange} onKeyDown={this.onInputchange} required />
                                        </CFormGroup>
                                        <CFormGroup className="pr-1" >
                                        ...
                                        <CFormGroup className="pr-1" style={{marginLeft: 'auto',marginRight: '0'}}  >
                                            ...
                                            <CButton
                                                ...
                                                variant="outline"
                                                shape="square"
                                                onClick={() => this.onDeleteItem(this.state.IdTipoEntrega)}
