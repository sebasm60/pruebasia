...
class BusquedaUbicacion extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    onSubmitFormFind() {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('Ubicaciones', '?page=1&limit=50&search=' + this.state.textBuscar.trim() + '', this.props.history).then(listdata => {
                    this.setState({
    ...
    onOpenActions(item, filter) {
        ...
        switch (filter) {
            default:
                ...
                break;
            case "Ingreso":
                this.props.history.push('/Maestros/ubicaciones/_add');
                break;
            case "Consulta":
            case "Edicion":
                this.props.history.push('/Maestros/ubicaciones/' + item.IdUbicacion + '');
                break;
            case "Retiro":
                DeleteItem(item.IdUbicacion, 'Ubicaciones', this.props.history);
                break;
            case "Inactivar":
            case "Activar":
                const datasend = {
                ...
                UpdateItem(item.IdUbicacion, datasend, 'Ubicaciones;put', this.props.history, '').then(result => {
                    this.onSubmitFormFind();
                });
                break;
        }
                        ...
                        <CCard style={{ display: this.state.showLoading ? 'none' : 'flex' }}>
                            ...
                            <CCardBody>
                                ...
                                <CDataTable
                                    ...
                                    fields={[
                                        { key: 'IdUbicacion', label: 'IdUbicacion' },
