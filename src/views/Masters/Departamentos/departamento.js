...
class departamento extends Component {
    ...
    ObtenerMestros() {
        ...
        getListSelect('Paises', '?Inactivo=false', history).then(listdata => {
            let tempArray = [];
    ...
    onInputchange(event) {
        if (event.target.name === "IdDepartamento") {
            this.setState({
    ...
    onChangeInfo(id) {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('Departamentos', '/' + id, this.props.history).then(listdata => {
                    console.log(listdata);
                    this.setState({
                        "showLoading": false,
                        "IdDepartamento": listdata.IdDepartamento,
                        "Nombre": listdata.Nombre,
                        "paisId": this.state.listPais.find(i => i.value === listdata.IdPais),
                ...
                validateRequiredFields(this.state, camposRequired).then(result => {
                    if (result) {
                        ...
                        const datasend = {
                            ...
                            Inactivo: false,
                            IdPais:this.state.paisId.value,
                            FhCreacion: formatDateP(new Date(), 'yyyy-MM-DD'),
                            IdUser_Ing: localStorage.getItem('LoginUsers'),
                        }
                        AddItem(datasend, 'Departamentos;post', history, '').then(result => {
                            this.ObtenerMestros();
                        });
                    }
                });
            ...
            if (resultPermiss) {
                ...
                validateRequiredFields(this.state, camposRequired).then(result => {
                    if (result) {
                        ...
                        UpdateItem(this.state.IdDepartamento, datasend, 'Departamentos;put', history, '').then(result => {
                            this.onChangeInfo(this.state.IdDepartamento);
                        });
                    }
                });
                this.setState({
    ...
    onSubmitFormDelete() {
        DeleteItem(this.state.IdDepartamento, 'Departamentos', this.props.history, '/Maestros/BusquedaDepartamentos');
    }

    onGoNewItem() {
        localStorage.setItem('ActionItem', '');
        this.props.history.push('/Maestros/BusquedaDepartamentos');
    }
    ...
    getTitle() {
        if (this.state.id === '_add') {
            return "Agregar DEPARTAMENTOS"
        } else {
            return "Consultar / Modificar / Eliminar DEPARTAMENTOS"
        }
    }
                                ...
                                <CCol md="12">
                                    <CForm inline>
                                        <CFormGroup className="pr-1" >
                                            <FloatingLabelInput id="IdDepartamento" name="IdDepartamento" style={{ paddingLeft: '2%' }} label="Id.Departamento" value={this.state.IdDepartamento} onChange={this.onInputchange} onKeyDown={this.onInputchange} required />
                                        </CFormGroup>
                                        <CFormGroup className="pr-1" >
                                        ...
                                        <CFormGroup className="pr-1" >
                                            <div style={{ width: '324px' }} >
                                                <Select
                                                    options={this.state.listPais}
                                                    styles={customSelectStyles}
                                                    value={this.state.paisId}
                                                    components={{
                                                        ValueContainer: CustomValueContainer
                                                    }}
                                                    isClearable
                                                    placeholder="Pais"
                                                    onChange={event => this.onSelectchange("paisId", event)}
                                                ></Select>
                                            </div>
                                        </CFormGroup>
                                        <CFormGroup className="pr-1" style={{marginLeft: 'auto',marginRight: '0'}}  >
                                            ...
                                            <CButton
                                                ...
                                                variant="outline"
                                                shape="square"
                                                onClick={() => this.onDeleteItem(this.state.IdDepartamento)}
