...
class BusquedaDepartamentos extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    onSubmitFormFind() {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                ...
                getListSelect('Departamentos', '?page=1&limit=50&search=' + this.state.textBuscar.trim() + '', this.props.history).then(listdata => {
    ...
    onOpenActions(item, filter) {
        ...
        switch (filter) {
            default:
                ...
                break;
            case "Ingreso":
                this.props.history.push('/Maestros/departamentos/_add');
                break;
            case "Consulta":
            case "Edicion":
            case "Retiro":
                this.props.history.push('/Maestros/departamentos/' + item.IdDepartamento + '');
                break;
            case "Inactivar":
            case "Activar":
                const datasend = {
                ...
                UpdateItem(item.IdDepartamento, datasend, 'Departamentos;put', this.props.history, '').then(result => {
                    this.onSubmitFormFind();
                });
                break;
        }
                        ...
                        <CCard style={{ display: this.state.showLoading ? 'none' : 'flex' }}>
                            ...
                            <CCardBody>
                                ...
                                <CDataTable
                                    ...
                                    fields={[
                                        { key: 'IdDepartamento', label: 'IdDepartamento' },
...
export default BusquedaDepartamentos