...
class MedioPago extends Component {
    ...
    onInputchange(event) {
        if (event.target.name === "IdMedioPago") {
            this.setState({
    ...
    onChangeInfo(id) {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('MediosPago', '/' + id, this.props.history).then(listdata => {
                    console.log(listdata);
                    this.setState({
                        "showLoading": false,
                        "IdMedioPago": listdata.IdMedioPago,
            ...
            if (resultPermiss) {
                ...
                validateRequiredFields(this.state, camposRequired).then(result => {
                    if (result) {
                        ...
                        UpdateItem(this.state.IdMedioPago, datasend, 'MediosPago;put', history, '').then(result => {
                            this.onChangeInfo(this.state.IdMedioPago);
                        });
                    }
                });
                this.setState({
    ...
    onSubmitFormDelete() {
        DeleteItem(this.state.IdMedioPago, 'MediosPago', this.props.history, '/Maestros/BusquedaMediosPago');
    }

    onGoNewItem() {
        localStorage.setItem('ActionItem', '');
        this.props.history.push('/Maestros/BusquedaMediosPago');
    }
    ...
    getTitle() {
        if (this.state.id === '_add') {
            return "Agregar MEDIOS DE PAGO"
        } else {
            return "Consultar / Modificar / Eliminar MEDIOS DE PAGO"
        }
    }
                                ...
                                <CCol md="12">
                                    <CForm inline>
                                        <CFormGroup className="pr-1" >
                                            <FloatingLabelInput id="IdMedioPago" name="IdMedioPago" style={{ paddingLeft: '2%' }} label="Id.MedioPago" value={this.state.IdMedioPago} onChange={this.onInputchange} onKeyDown={this.onInputchange} required />
                                        </CFormGroup>
                                        <CFormGroup className="pr-1" >
                                        ...
                                        <CFormGroup className="pr-1" style={{marginLeft: 'auto',marginRight: '0'}}  >
                                            ...
                                            <CButton
                                                ...
                                                variant="outline"
                                                shape="square"
                                                onClick={() => this.onDeleteItem(this.state.IdMedioPago)}
