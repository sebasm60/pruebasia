...
class BusquedaViasAdministracion extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    onSubmitFormFind() {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('VIASADMON', '?page=1&limit=50&search=' + this.state.textBuscar.trim() + '', this.props.history).then(listdata => {
    ...
    onOpenActions(item, filter) {
        ...
        switch (filter) {
            default:
                ...
                break;
            case "Ingreso":
                this.props.history.push('/Maestros/viasAdministracion/_add');
                break;
            case "Consulta":
            case "Edicion":
                this.props.history.push('/Maestros/viasAdministracion/' + item.IdViaAdmon + '');
                break;
            case "Retiro":
                DeleteItem(item.IdViaAdmon, 'VIASADMON', this.props.history);
                break;
            case "Inactivar":
            case "Activar":
                const datasend = {
                ...
                UpdateItem(item.IdViaAdmon, datasend, 'VIASADMON;put', this.props.history, '').then(result => {
                    this.onSubmitFormFind();
                });
                        ...
                        <CCard style={{ display: this.state.showLoading ? 'none' : 'flex' }}>
                            ...
                            <CCardBody>
                                ...
                                <CDataTable
                                    ...
                                    fields={[
                                        { key: 'IdViaAdmon', label: 'IdViaAdmon' },
...
export default BusquedaViasAdministracion