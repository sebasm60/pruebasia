...
class viasAdministracion extends Component {
    constructor(props) {
        super(props);
        this.state = {
            IdMenu: 'Menu-viasadmon',
    ...
    onInputchange(event) {
        if (event.target.name === "IdViaAdmon") {
            this.setState({
    ...
    onChangeInfo(id) {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('VIASADMON', '/' + id, this.props.history).then(listdata => {
                    console.log(listdata);
                    this.setState({
                        "showLoading": false,
                        "IdViaAdmon": listdata.IdViaAdmon,
            ...
            if (resultPermiss) {
                ...
                validateRequiredFields(this.state, camposRequired).then(result => {
                    if (result) {
                        ...
                        UpdateItem(this.state.IdViaAdmon, datasend, 'VIASADMON;put', history, '').then(result => {
                            this.onChangeInfo(this.state.IdViaAdmon);
                        });
                    }
                });
                this.setState({
    ...
    onSubmitFormDelete() {
        DeleteItem(this.state.IdViaAdmon, 'VIASADMON', this.props.history, '/Maestros/BusquedaViasAdmin');
    }

    onGoNewItem() {
        localStorage.setItem('ActionItem', '');
        this.props.history.push('/Maestros/BusquedaViasAdmin');
    }
    ...
    getTitle() {
        if (this.state.id === '_add') {
            return "Agregar VIAS ADMINISTRACION"
        } else {
            return "Consultar / Modificar / Eliminar VIAS ADMINISTRACION"
        }
    }
                                ...
                                <CCol md="12">
                                    <CForm inline>
                                        <CFormGroup className="pr-1" >
                                            <FloatingLabelInput id="IdViaAdmon" name="IdViaAdmon" style={{ paddingLeft: '2%' }} label="Id.ViaAdmon" value={this.state.IdViaAdmon} onChange={this.onInputchange} onKeyDown={this.onInputchange} required />
                                        </CFormGroup>
                                        <CFormGroup className="pr-1" >
                                        ...
                                        <CFormGroup className="pr-1" style={{marginLeft: 'auto',marginRight: '0'}}  >
                                            ...
                                            <CButton
                                                ...
                                                variant="outline"
                                                shape="square"
                                                onClick={() => this.onDeleteItem(this.state.IdViaAdmon)}
...
export default viasAdministracion