...
class BusquedaRoles extends Component {
    constructor(props) {
        super(props);
        this.state = {
            IdMenu: 'Menu-roles',
    ...
    onSubmitFormFind() {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('Roles', '?page=1&limit=50&search=' + this.state.textBuscar.trim() + '', this.props.history).then(listdata => {
    ...
    onOpenActions(item, filter) {
        ...
        switch (filter) {
            default:
                ...
                break;
            case "Ingreso":
                this.props.history.push('/Maestros/Roles/_add');
                break;
            case "Consulta":
            case "Edicion":
                this.props.history.push('/Maestros/Roles/' + item.IdRol + '');
                break;
            case "Retiro":
                DeleteItem(item.IdRol, 'Roles', this.props.history);
                break;
            case "Inactivar":
            case "Activar":
                const datasend = {
                ...
                UpdateItem(item.IdRol, datasend, 'Roles;put', this.props.history, '').then(result => {
                 this.onSubmitFormFind();
                });
