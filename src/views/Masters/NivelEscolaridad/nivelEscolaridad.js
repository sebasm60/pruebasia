...
class nivelEscolaridad extends Component {
    constructor(props) {
        super(props);
        this.state = {
            IdMenu: 'Menu-nivelesescol',
    ...
    onInputchange(event) {
        if (event.target.name === "IdNivelEscol") {
            this.setState({
    ...
    onChangeInfo(id) {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('NivelEscolaridad', '/' + id, this.props.history).then(listdata => {
                    console.log(listdata);
                    this.setState({
                        "showLoading": false,
                        "IdNivelEscol": listdata.IdNivelEscol,
            ...
            if (resultPermiss) {
                ...
                validateRequiredFields(this.state, camposRequired).then(result => {
                    if (result) {
                        ...
                        const datasend = {
                            IdNivelEscol: this.state.IdNivelEscol,
                            Nombre: this.state.Nombre,
                            Inactivo: false,
                            FhCreacion: formatDateP(new Date(), 'yyyy-MM-DD'),
                            IdUser_Ing: localStorage.getItem('LoginUsers'),
                        }
                        AddItem(datasend, 'NivelEscolaridad;post', history, '').then(result => {
                            this.ObtenerMestros();
                        });
                    }
                });
            ...
            if (resultPermiss) {
                ...
                validateRequiredFields(this.state, camposRequired).then(result => {
                    if (result) {
                        ...
                        UpdateItem(this.state.IdNivelEscol, datasend, 'NivelEscolaridad;put', history, '').then(result => {
                            this.onChangeInfo(this.state.IdNivelEscol);
                        });
                    }
                });
                this.setState({
    ...
    onSubmitFormDelete() {
        DeleteItem(this.state.IdNivelEscol, 'NivelEscolaridad', this.props.history, '/Maestros/BusquedaNivelEsc');
    }

    onGoNewItem() {
        localStorage.setItem('ActionItem', '');
        this.props.history.push('/Maestros/BusquedaNivelEsc');
    }
    ...
    getTitle() {
        if (this.state.id === '_add') {
            return "Agregar NIVEL ESCOLARIDAD"
        } else {
            return "Consultar / Modificar / Eliminar NIVEL ESCOLARIDAD"
        }
    }
                                ...
                                <CCol md="12">
                                    <CForm inline>
                                        <CFormGroup className="pr-1" >
                                            <FloatingLabelInput id="IdNivelEscol" name="IdNivelEscol" style={{ paddingLeft: '2%' }} label="Id.NivelEsco" value={this.state.IdNivelEscol} onChange={this.onInputchange} onKeyDown={this.onInputchange} required />
                                        </CFormGroup>
                                        <CFormGroup className="pr-1" >
                                        ...
                                        <CFormGroup className="pr-1" style={{marginLeft: 'auto',marginRight: '0'}}  >
                                            ...
                                            <CButton
                                                ...
                                                variant="outline"
                                                shape="square"
                                                onClick={() => this.onDeleteItem(this.state.IdNivelEscol)}
