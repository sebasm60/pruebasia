...
class BusquedaATC extends Component {
    constructor(props) {
        super(props);
        this.state = {
            IdMenu: 'Menu-atc',
    ...
    onSubmitFormFind() {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('ATC', '?page=1&limit=50&search=' + this.state.textBuscar.trim() + '', this.props.history).then(listdata => {
    ...
    onOpenActions(item, filter) {
        ...
        switch (filter) {
            default:
                ...
                break;
            case "Ingreso":
                this.props.history.push('/Maestros/atc/_add');
                break;
            case "Consulta":
            case "Edicion":
            case "Retiro":
                this.props.history.push('/Maestros/atc/' + item.IdATC + '');
                break;
            case "Inactivar":
            case "Activar":
                const datasend = {
                ...
                UpdateItem(item.IdATC, datasend, 'ATC;put', this.props.history, '').then(result => {
                    this.onSubmitFormFind();
                });
