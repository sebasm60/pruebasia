...
class atc extends Component {
    ...
    onSubmitFormDelete() {
        DeleteItem(this.state.IdATC, 'ATC', this.props.history, '/Maestros/BusquedaATC');
    }
    ...
    onChangeInfo(id) {
        ...
        getPermissions(this.state.IdMenu, 'Consulta', this.props.history).then(resultPermiss => {
            if (resultPermiss) {
                getListSelect('ATC', '/' + id, this.props.history).then(listdata => {
                    console.log(listdata);
                    this.setState({
                        "showLoading": false,
                        "IdATC": listdata.IdATC,
            ...
            if (resultPermiss) {
                ...
                validateRequiredFields(this.state, camposRequired).then(result => {
                    if (result) {
                        ...
                        UpdateItem(this.state.IdATC, datasend, 'ATC;put', history, '').then(result => {
                            this.onChangeInfo(this.state.IdATC);
                        });
                    }
                });
                this.setState({
    ...
    onGoNewItem() {
        ...
        this.props.history.push('/Maestros/BusquedaATC');
    }
    ...
    getTitle() {
        if (this.state.id === '_add') {
            ...
            return "Consultar / Modificar / Eliminar ATC"
        }
    }
                                ...
                                <CCol md="12">
                                    <CForm inline>
                                        <CFormGroup className="pr-1" >
                                            <FloatingLabelInput id="IdATC" name="IdATC" style={{ paddingLeft: '2%' }} label="Id.ATC" value={this.state.IdATC} onChange={this.onInputchange} onKeyDown={this.onInputchange} required />
                                        </CFormGroup>
                                        <CFormGroup className="pr-1" >
                                        ...
                                        <CFormGroup className="pr-1" style={{ marginLeft: 'auto', marginRight: '0' }}  >
                                            ...
                                            <CButton
                                                ...
                                                variant="outline"
                                                shape="square"
                                                onClick={() => this.onDeleteItem(this.state.IdATC)}
